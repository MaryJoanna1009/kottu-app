...
def get_inventory(shop_id: int):
    try:
        if USE_POSTGRES:
            ...
            conn.close()
        print(f"✅ Returning {len(res)} items for shop_id={shop_id}")
        return res
    except Exception as e:
...
def add_item(shop_id: int, name: str, price: int, stock: int):
    try:
        if USE_POSTGRES:
            ...
            conn.commit(); conn.close()
        print(f"✅ Added item '{name}' for shop_id={shop_id}")
        return {"status": "added", "shop_id": shop_id, "name": name}
    except Exception as e:
...
def get_udhaar_ledger(shop_id: int):
    try:
        ...
        print(f"❌ get_udhaar_ledger error: {e}")
        return []
...
@app.get("/")
def root(): return {"status": "✅ KOTTU Multi-Shop Live", "mode": "PostgreSQL" if USE_POSTGRES else "SQLite"}